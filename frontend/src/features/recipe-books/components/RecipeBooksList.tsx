...
export function RecipeBooksList({ groupId }: RecipeBooksListProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">Recipe books</h2>
        {canCreate ? <CreateRecipeBookDialog groupId={groupId} /> : null}
      </div>

      {recipeBooksQuery.data?.length === 0 ? (
        <p className="text-muted-foreground">No recipe books yet.</p>
      ) : null}

      <div className="grid gap-4 sm:grid-cols-2">
        {recipeBooksQuery.data?.map((recipeBook) => (
          <Link
            key={recipeBook.id}
